...
def main(input_dir: str, output_dir: str) -> None:
    ...
    S65_total["S65E_tot_cmd"] = S65_total[["S65E_S_FLOW_cfs", "S65EX1_S_FLOW_cfs"]].sum(
        axis=1
    )
    ...
    S65_total_TP = pd.read_csv(
        f"{input_dir}/water_quality_S65E_PHOSPHATE, TOTAL AS P_Interpolated.csv"
    )
    S71_TP = pd.read_csv(
        f"{input_dir}/water_quality_S71_PHOSPHATE, TOTAL AS P_Interpolated.csv"
    )
    S72_TP = pd.read_csv(
        f"{input_dir}/water_quality_S72_PHOSPHATE, TOTAL AS P_Interpolated.csv"
    )
    S84_TP = pd.read_csv(
        f"{input_dir}/water_quality_S84_PHOSPHATE, TOTAL AS P_Interpolated.csv"
    )
    S127_TP = pd.read_csv(
        f"{input_dir}/water_quality_S127_PHOSPHATE, TOTAL AS P_Interpolated.csv"
    )
    S133_TP = pd.read_csv(
        f"{input_dir}/water_quality_S133_PHOSPHATE, TOTAL AS P_Interpolated.csv"
    )
    S135_TP = pd.read_csv(
        f"{input_dir}/water_quality_S135_PHOSPHATE, TOTAL AS P_Interpolated.csv"
    )
    S154_TP = pd.read_csv(
        f"{input_dir}/water_quality_S154_PHOSPHATE, TOTAL AS P_Interpolated.csv"
    )
    S191_TP = pd.read_csv(
        f"{input_dir}/water_quality_S191_PHOSPHATE, TOTAL AS P_Interpolated.csv"
    )
    S308_TP = pd.read_csv(
        f"{input_dir}/water_quality_S308C_PHOSPHATE, TOTAL AS P_Interpolated.csv"
    )
    FISHP_TP = pd.read_csv(
        f"{input_dir}/water_quality_FECSR78_PHOSPHATE, TOTAL AS P_Interpolated.csv"
    )
    L8_TP = pd.read_csv(
        f"{input_dir}/water_quality_CULV10A_PHOSPHATE, TOTAL AS P_Interpolated.csv"
    )
    S4_TP = pd.read_csv(
        f"{input_dir}/water_quality_S4_PHOSPHATE, TOTAL AS P_Interpolated.csv"
    )
    ...
    TP_Loads_In["S65_P_Ld"] = (
        Flow_df["S65_Q"] * TP_df["S65_TP"] * 1000
    )  # (m3/d * mg/L * 1000 = mg/d)
    ...
    for i in range(len(C44RO_df.index)):
        if (
            S79["Q_cmd"].iloc[i]
            - Flow_df["S77_Out"].iloc[i]
            + Flow_df["S77_In"].iloc[i]
            < 0
        ):
            C43RO[i] = 0
        else:
    ...
    for i in range(len(C44RO_df.index)):
        if (
            S80["Q_cmd"].iloc[i]
            - Flow_df["S308_Out"].iloc[i]
            + Flow_df["S308_In"].iloc[i]
            < 0
        ):
            C44RO[i] = 0
        else:
    ...
    Net_Inflows["Net_Inflows"] = Flow_df["Netflows"] / (
        0.0283168466 * 86400
    )  # cmd to cfs
    ...
    RFVol["RFVol_acft"] = (RF_data["average_rainfall"].values / 12) * LO_Stg_Sto_SA_df[
        "SA_acres"
    ].values
    date_reference = RFVol["date"].iloc[0]
    date_inserts = [
        date_reference - datetime.timedelta(days=2),
        date_reference - datetime.timedelta(days=1),
    ]
    ...
    ETVol["ETVol_acft"] = (ET_data["average_ETPI"].values / 12) * LO_Stg_Sto_SA_df[
        "SA_acres"
    ].values
    ...
    Water_Temp_data = DF_Date_Range(
        Water_Temp_data, St_Yr, St_M, St_D, En_Yr, En_M, En_D
    )
    water_temp_filter_cols = [
        "L001_WaterT",
        "L005_WaterT",
        "L006_WaterT",
        "LZ40_WaterT",
    ]
    Water_Temp_data["WaterT_Mean"] = Water_Temp_data[water_temp_filter_cols].mean(
        axis=1
    )
    ...
    WaterT_pred_df["L001_WaterT_pred"] = (
        1.862667 + 0.936899 * L001_AirT["L001_AIRT_Degrees Celsius"].values
    )
    WaterT_pred_df["L005_WaterT_pred"] = (
        1.330211 + 0.909713 * L005_AirT["L005_AIRT_Degrees Celsius"].values
    )
    WaterT_pred_df["L006_WaterT_pred"] = (
        -0.88564 + 1.01585 * L006_AirT["L006_AIRT_Degrees Celsius"].values
    )
    WaterT_pred_df["LZ40_WaterT_pred"] = (
        0.388231 + 0.980154 * LZ40_AirT["LZ40_AIRT_Degrees Celsius"].values
    )
    water_t_pred_filter_cols = [
        "L001_WaterT_pred",
        "L005_WaterT_pred",
        "L006_WaterT_pred",
        "LZ40_WaterT_pred",
    ]
    WaterT_pred_df["WaterT_pred_Mean"] = WaterT_pred_df[water_t_pred_filter_cols].mean(
        axis=1
    )
    ...
    Filled_WaterT = pd.concat([Filled_WaterT_1df, Filled_WaterT_2df]).reset_index(
        drop=True
    )
    ...
    L001_TP_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L001_PHOSPHATE, TOTAL AS P_Interpolated.csv"
    )
    L004_TP_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L004_PHOSPHATE, TOTAL AS P_Interpolated.csv"
    )
    L005_TP_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L005_PHOSPHATE, TOTAL AS P_Interpolated.csv"
    )
    L006_TP_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L006_PHOSPHATE, TOTAL AS P_Interpolated.csv"
    )
    L007_TP_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L007_PHOSPHATE, TOTAL AS P_Interpolated.csv"
    )
    L008_TP_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L008_PHOSPHATE, TOTAL AS P_Interpolated.csv"
    )
    LZ40_TP_Inter = pd.read_csv(
        f"{input_dir}/water_quality_LZ40_PHOSPHATE, TOTAL AS P_Interpolated.csv"
    )

    L001_TP_Inter.columns = L001_TP_Inter.columns.str.replace(
        "Data", "Data_L001_TP_Inter"
    )
    L004_TP_Inter.columns = L004_TP_Inter.columns.str.replace(
        "Data", "Data_L004_TP_Inter"
    )
    L005_TP_Inter.columns = L005_TP_Inter.columns.str.replace(
        "Data", "Data_L005_TP_Inter"
    )
    L006_TP_Inter.columns = L006_TP_Inter.columns.str.replace(
        "Data", "Data_L006_TP_Inter"
    )
    L007_TP_Inter.columns = L007_TP_Inter.columns.str.replace(
        "Data", "Data_L007_TP_Inter"
    )
    L008_TP_Inter.columns = L008_TP_Inter.columns.str.replace(
        "Data", "Data_L008_TP_Inter"
    )
    LZ40_TP_Inter.columns = LZ40_TP_Inter.columns.str.replace(
        "Data", "Data_LZ40_TP_Inter"
    )
    ...
    LO_TP_data_Inter = LO_TP_data_Inter.loc[
        :, ~LO_TP_data_Inter.columns.str.startswith("Unnamed")
    ]
    ...
    L001_OP_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L001_PHOSPHATE, ORTHO AS P_Interpolated.csv"
    )
    L004_OP_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L004_PHOSPHATE, ORTHO AS P_Interpolated.csv"
    )
    L005_OP_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L005_PHOSPHATE, ORTHO AS P_Interpolated.csv"
    )
    L006_OP_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L006_PHOSPHATE, ORTHO AS P_Interpolated.csv"
    )
    L007_OP_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L007_PHOSPHATE, ORTHO AS P_Interpolated.csv"
    )
    L008_OP_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L008_PHOSPHATE, ORTHO AS P_Interpolated.csv"
    )
    LZ40_OP_Inter = pd.read_csv(
        f"{input_dir}/water_quality_LZ40_PHOSPHATE, ORTHO AS P_Interpolated.csv"
    )

    L001_OP_Inter.columns = L001_OP_Inter.columns.str.replace(
        "Data", "Data_L001_OP_Inter"
    )
    L004_OP_Inter.columns = L004_OP_Inter.columns.str.replace(
        "Data", "Data_L004_OP_Inter"
    )
    L005_OP_Inter.columns = L005_OP_Inter.columns.str.replace(
        "Data", "Data_L005_OP_Inter"
    )
    L006_OP_Inter.columns = L006_OP_Inter.columns.str.replace(
        "Data", "Data_L006_OP_Inter"
    )
    L007_OP_Inter.columns = L007_OP_Inter.columns.str.replace(
        "Data", "Data_L007_OP_Inter"
    )
    L008_OP_Inter.columns = L008_OP_Inter.columns.str.replace(
        "Data", "Data_L008_OP_Inter"
    )
    LZ40_OP_Inter.columns = LZ40_OP_Inter.columns.str.replace(
        "Data", "Data_LZ40_OP_Inter"
    )
    ...
    LO_OP_data_Inter = LO_OP_data_Inter.loc[
        :, ~LO_OP_data_Inter.columns.str.startswith("Unnamed")
    ]
    LO_OP_data_Inter["Mean_OP"] = LO_OP_data_Inter.mean(axis=1, numeric_only=True)
    LO_OP_data_Inter = DF_Date_Range(
        LO_OP_data_Inter, St_Yr, St_M, St_D, En_Yr, En_M, En_D
    )
    LO_OP_data_Inter.to_csv(f"{output_dir}/LO_OP.csv", index=False)

    # Create File (N_OP) (L001, L005, L008)
    n_op = LO_OP_data_Inter[
        ["date", "Data_L001_OP_Inter", "Data_L005_OP_Inter", "Data_L008_OP_Inter"]
    ]
    n_op["OP"] = n_op.mean(axis=1, numeric_only=True) * 1000  # mg/L to mg/m3
    n_op.drop(
        ["Data_L001_OP_Inter", "Data_L005_OP_Inter", "Data_L008_OP_Inter"],
        axis=1,
        inplace=True,
    )
    ...
    L001_NH4_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L001_AMMONIA-N_Interpolated.csv"
    )
    L004_NH4_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L004_AMMONIA-N_Interpolated.csv"
    )
    L005_NH4_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L005_AMMONIA-N_Interpolated.csv"
    )
    L006_NH4_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L006_AMMONIA-N_Interpolated.csv"
    )
    L007_NH4_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L007_AMMONIA-N_Interpolated.csv"
    )
    L008_NH4_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L008_AMMONIA-N_Interpolated.csv"
    )
    LZ40_NH4_Inter = pd.read_csv(
        f"{input_dir}/water_quality_LZ40_AMMONIA-N_Interpolated.csv"
    )

    L001_NH4_Inter.columns = L001_NH4_Inter.columns.str.replace(
        "Data", "Data_L001_NH4_Inter"
    )
    L004_NH4_Inter.columns = L004_NH4_Inter.columns.str.replace(
        "Data", "Data_L004_NH4_Inter"
    )
    L005_NH4_Inter.columns = L005_NH4_Inter.columns.str.replace(
        "Data", "Data_L005_NH4_Inter"
    )
    L006_NH4_Inter.columns = L006_NH4_Inter.columns.str.replace(
        "Data", "Data_L006_NH4_Inter"
    )
    L007_NH4_Inter.columns = L007_NH4_Inter.columns.str.replace(
        "Data", "Data_L007_NH4_Inter"
    )
    L008_NH4_Inter.columns = L008_NH4_Inter.columns.str.replace(
        "Data", "Data_L008_NH4_Inter"
    )
    LZ40_NH4_Inter.columns = LZ40_NH4_Inter.columns.str.replace(
        "Data", "Data_LZ40_NH4_Inter"
    )

    LO_NH4_data_Inter = pd.merge(L001_NH4_Inter, L004_NH4_Inter, how="left", on="date")
    LO_NH4_data_Inter = pd.merge(
        LO_NH4_data_Inter, L005_NH4_Inter, how="left", on="date"
    )
    LO_NH4_data_Inter = pd.merge(
        LO_NH4_data_Inter, L006_NH4_Inter, how="left", on="date"
    )
    LO_NH4_data_Inter = pd.merge(
        LO_NH4_data_Inter, L007_NH4_Inter, how="left", on="date"
    )
    LO_NH4_data_Inter = pd.merge(
        LO_NH4_data_Inter, L008_NH4_Inter, how="left", on="date"
    )
    LO_NH4_data_Inter = pd.merge(
        LO_NH4_data_Inter, LZ40_NH4_Inter, how="left", on="date"
    )
    ...
    L001_NO_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L001_NITRATE+NITRITE-N_Interpolated.csv"
    )
    L004_NO_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L004_NITRATE+NITRITE-N_Interpolated.csv"
    )
    L005_NO_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L005_NITRATE+NITRITE-N_Interpolated.csv"
    )
    L006_NO_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L006_NITRATE+NITRITE-N_Interpolated.csv"
    )
    L007_NO_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L007_NITRATE+NITRITE-N_Interpolated.csv"
    )
    L008_NO_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L008_NITRATE+NITRITE-N_Interpolated.csv"
    )
    LZ40_NO_Inter = pd.read_csv(
        f"{input_dir}/water_quality_LZ40_NITRATE+NITRITE-N_Interpolated.csv"
    )

    L001_NO_Inter.columns = L001_NO_Inter.columns.str.replace(
        "Data", "Data_L001_NO_Inter"
    )
    L004_NO_Inter.columns = L004_NO_Inter.columns.str.replace(
        "Data", "Data_L004_NO_Inter"
    )
    L005_NO_Inter.columns = L005_NO_Inter.columns.str.replace(
        "Data", "Data_L005_NO_Inter"
    )
    L006_NO_Inter.columns = L006_NO_Inter.columns.str.replace(
        "Data", "Data_L006_NO_Inter"
    )
    L007_NO_Inter.columns = L007_NO_Inter.columns.str.replace(
        "Data", "Data_L007_NO_Inter"
    )
    L008_NO_Inter.columns = L008_NO_Inter.columns.str.replace(
        "Data", "Data_L008_NO_Inter"
    )
    LZ40_NO_Inter.columns = LZ40_NO_Inter.columns.str.replace(
        "Data", "Data_LZ40_NO_Inter"
    )
    ...
    LO_NO_data_Inter = LO_NO_data_Inter.loc[
        :, ~LO_NO_data_Inter.columns.str.startswith("Unnamed")
    ]
    ...
    # Create File (LO_DIN)
    date_DIN = pd.date_range(
        start="%s/%s/%s" % (St_M, St_D, St_Yr),
        end="%s/%s/%s" % (En_M, En_D, En_Yr),
        freq="D",
    )
    LO_DIN = pd.DataFrame(date_DIN, columns=["date"])
    LO_NH4_Clean_Inter["date"] = LO_NH4_Clean_Inter.index
    LO_NH4_Clean_Inter = DF_Date_Range(
        LO_NH4_Clean_Inter, St_Yr, St_M, St_D, En_Yr, En_M, En_D
    )
    LO_NO_data_Inter["date"] = LO_NO_data_Inter.index
    LO_NO_Clean_Inter = DF_Date_Range(
        LO_NO_data_Inter, St_Yr, St_M, St_D, En_Yr, En_M, En_D
    )
    ...
    n_din["NH4"] = LO_NH4_Clean_Inter[
        ["date", "Data_L001_NH4_Inter", "Data_L005_NH4_Inter", "Data_L008_NH4_Inter"]
    ].mean(axis=1, numeric_only=True)
    n_din["NO"] = (
        LO_NO_Clean_Inter[
            ["date", "Data_L001_NO_Inter", "Data_L005_NO_Inter", "Data_L008_NO_Inter"]
        ].mean(axis=1, numeric_only=True)
        * 1000
    )  # mg/L to mg/m3
    ...
    L001_DO_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L001_DISSOLVED OXYGEN_Interpolated.csv"
    )
    L004_DO_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L004_DISSOLVED OXYGEN_Interpolated.csv"
    )
    L005_DO_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L005_DISSOLVED OXYGEN_Interpolated.csv"
    )
    L006_DO_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L006_DISSOLVED OXYGEN_Interpolated.csv"
    )
    L007_DO_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L007_DISSOLVED OXYGEN_Interpolated.csv"
    )
    L008_DO_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L008_DISSOLVED OXYGEN_Interpolated.csv"
    )
    LZ40_DO_Inter = pd.read_csv(
        f"{input_dir}/water_quality_LZ40_DISSOLVED OXYGEN_Interpolated.csv"
    )

    L001_DO_Inter.columns = L001_DO_Inter.columns.str.replace(
        "Data", "Data_L001_DO_Inter"
    )
    L004_DO_Inter.columns = L004_DO_Inter.columns.str.replace(
        "Data", "Data_L004_DO_Inter"
    )
    L005_DO_Inter.columns = L005_DO_Inter.columns.str.replace(
        "Data", "Data_L005_DO_Inter"
    )
    L006_DO_Inter.columns = L006_DO_Inter.columns.str.replace(
        "Data", "Data_L006_DO_Inter"
    )
    L007_DO_Inter.columns = L007_DO_Inter.columns.str.replace(
        "Data", "Data_L007_DO_Inter"
    )
    L008_DO_Inter.columns = L008_DO_Inter.columns.str.replace(
        "Data", "Data_L008_DO_Inter"
    )
    LZ40_DO_Inter.columns = LZ40_DO_Inter.columns.str.replace(
        "Data", "Data_LZ40_DO_Inter"
    )
    ...
    LO_DO_data_Inter = LO_DO_data_Inter.loc[
        :, ~LO_DO_data_Inter.columns.str.startswith("Unnamed")
    ]
    # Read clean LO_DO data
    LO_DO_data_Inter["Mean_DO"] = LO_DO_data_Inter.mean(axis=1, numeric_only=True)
    LO_DO_data_Inter = DF_Date_Range(
        LO_DO_data_Inter, St_Yr, St_M, St_D, En_Yr, En_M, En_D
    )
    ...
    L001_RADT.columns = L001_RADT.columns.str.replace(
        "Unnamed: 0", "Unnamed: 0_L001_RADT"
    )
    L005_RADT.columns = L005_RADT.columns.str.replace(
        "Unnamed: 0", "Unnamed: 0_L005_RADT"
    )
    L006_RADT.columns = L006_RADT.columns.str.replace(
        "Unnamed: 0", "Unnamed: 0_L006_RADT"
    )
    LZ40_RADT.columns = LZ40_RADT.columns.str.replace(
        "Unnamed: 0", "Unnamed: 0_LZ40_RADT"
    )
    ...
    L001_RADP.columns = L001_RADP.columns.str.replace(
        "Unnamed: 0", "Unnamed: 0_L001_RADP"
    )
    L005_RADP.columns = L005_RADP.columns.str.replace(
        "Unnamed: 0", "Unnamed: 0_L005_RADP"
    )
    L006_RADP.columns = L006_RADP.columns.str.replace(
        "Unnamed: 0", "Unnamed: 0_L006_RADP"
    )
    LZ40_RADP.columns = LZ40_RADP.columns.str.replace(
        "Unnamed: 0", "Unnamed: 0_LZ40_RADP"
    )
    ...
    L001_Chla_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L001_CHLOROPHYLL-A, CORRECTED_Interpolated.csv"
    )
    L004_Chla_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L004_CHLOROPHYLL-A, CORRECTED_Interpolated.csv"
    )
    L005_Chla_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L005_CHLOROPHYLL-A, CORRECTED_Interpolated.csv"
    )
    L006_Chla_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L006_CHLOROPHYLL-A, CORRECTED_Interpolated.csv"
    )
    L007_Chla_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L007_CHLOROPHYLL-A, CORRECTED_Interpolated.csv"
    )
    L008_Chla_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L008_CHLOROPHYLL-A, CORRECTED_Interpolated.csv"
    )
    LZ40_Chla_Inter = pd.read_csv(
        f"{input_dir}/water_quality_LZ40_CHLOROPHYLL-A, CORRECTED_Interpolated.csv"
    )

    L001_Chla_Inter.columns = L001_Chla_Inter.columns.str.replace(
        "Data", "Data_L001_Chla_Inter"
    )
    L004_Chla_Inter.columns = L004_Chla_Inter.columns.str.replace(
        "Data", "Data_L004_Chla_Inter"
    )
    L005_Chla_Inter.columns = L005_Chla_Inter.columns.str.replace(
        "Data", "Data_L005_Chla_Inter"
    )
    L006_Chla_Inter.columns = L006_Chla_Inter.columns.str.replace(
        "Data", "Data_L006_Chla_Inter"
    )
    L007_Chla_Inter.columns = L007_Chla_Inter.columns.str.replace(
        "Data", "Data_L007_Chla_Inter"
    )
    L008_Chla_Inter.columns = L008_Chla_Inter.columns.str.replace(
        "Data", "Data_L008_Chla_Inter"
    )
    LZ40_Chla_Inter.columns = LZ40_Chla_Inter.columns.str.replace(
        "Data", "Data_LZ40_Chla_Inter"
    )

    LO_Chla_data_Inter = pd.merge(
        L001_Chla_Inter, L004_Chla_Inter, how="left", on="date"
    )
    LO_Chla_data_Inter = pd.merge(
        LO_Chla_data_Inter, L005_Chla_Inter, how="left", on="date"
    )
    LO_Chla_data_Inter = pd.merge(
        LO_Chla_data_Inter, L006_Chla_Inter, how="left", on="date"
    )
    LO_Chla_data_Inter = pd.merge(
        LO_Chla_data_Inter, L007_Chla_Inter, how="left", on="date"
    )
    LO_Chla_data_Inter = pd.merge(
        LO_Chla_data_Inter, L008_Chla_Inter, how="left", on="date"
    )
    LO_Chla_data_Inter = pd.merge(
        LO_Chla_data_Inter, LZ40_Chla_Inter, how="left", on="date"
    )
    LO_Chla_data_Inter = LO_Chla_data_Inter.loc[
        :, ~LO_Chla_data_Inter.columns.str.startswith("Unnamed")
    ]
    ...
    L001_Chla_LC_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L001_CHLOROPHYLL-A(LC)_Interpolated.csv"
    )
    L004_Chla_LC_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L004_CHLOROPHYLL-A(LC)_Interpolated.csv"
    )
    L005_Chla_LC_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L005_CHLOROPHYLL-A(LC)_Interpolated.csv"
    )
    L006_Chla_LC_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L006_CHLOROPHYLL-A(LC)_Interpolated.csv"
    )
    L007_Chla_LC_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L007_CHLOROPHYLL-A(LC)_Interpolated.csv"
    )
    L008_Chla_LC_Inter = pd.read_csv(
        f"{input_dir}/water_quality_L008_CHLOROPHYLL-A(LC)_Interpolated.csv"
    )
    LZ40_Chla_LC_Inter = pd.read_csv(
        f"{input_dir}/water_quality_LZ40_CHLOROPHYLL-A(LC)_Interpolated.csv"
    )

    L001_Chla_LC_Inter.columns = L001_Chla_LC_Inter.columns.str.replace(
        "Data", "Data_L001_Chla_LC_Inter"
    )
    L004_Chla_LC_Inter.columns = L004_Chla_LC_Inter.columns.str.replace(
        "Data", "Data_L004_Chla_LC_Inter"
    )
    L005_Chla_LC_Inter.columns = L005_Chla_LC_Inter.columns.str.replace(
        "Data", "Data_L005_Chla_LC_Inter"
    )
    L006_Chla_LC_Inter.columns = L006_Chla_LC_Inter.columns.str.replace(
        "Data", "Data_L006_Chla_LC_Inter"
    )
    L007_Chla_LC_Inter.columns = L007_Chla_LC_Inter.columns.str.replace(
        "Data", "Data_L007_Chla_LC_Inter"
    )
    L008_Chla_LC_Inter.columns = L008_Chla_LC_Inter.columns.str.replace(
        "Data", "Data_L008_Chla_LC_Inter"
    )
    LZ40_Chla_LC_Inter.columns = LZ40_Chla_LC_Inter.columns.str.replace(
        "Data", "Data_LZ40_Chla_LC_Inter"
    )

    LO_Chla_LC_data_Inter = pd.merge(
        L001_Chla_LC_Inter, L004_Chla_LC_Inter, how="left", on="date"
    )
    LO_Chla_LC_data_Inter = pd.merge(
        LO_Chla_LC_data_Inter, L005_Chla_LC_Inter, how="left", on="date"
    )
    LO_Chla_LC_data_Inter = pd.merge(
        LO_Chla_LC_data_Inter, L006_Chla_LC_Inter, how="left", on="date"
    )
    LO_Chla_LC_data_Inter = pd.merge(
        LO_Chla_LC_data_Inter, L007_Chla_LC_Inter, how="left", on="date"
    )
    LO_Chla_LC_data_Inter = pd.merge(
        LO_Chla_LC_data_Inter, L008_Chla_LC_Inter, how="left", on="date"
    )
    LO_Chla_LC_data_Inter = pd.merge(
        LO_Chla_LC_data_Inter, LZ40_Chla_LC_Inter, how="left", on="date"
    )
    LO_Chla_LC_data_Inter = LO_Chla_LC_data_Inter.loc[
        :, ~LO_Chla_LC_data_Inter.columns.str.startswith("Unnamed")
    ]
    # Read clean LO_Chla_LC data
    LO_Chla_LC_data_Inter["Mean_Chla_LC"] = LO_Chla_LC_data_Inter.mean(
        axis=1, numeric_only=True
    )
    LO_Chla_LC_data_Inter.to_csv(
        f"{output_dir}/LO_Chla_LC_Clean_daily.csv", index=False
    )
    # Monthly
    ...
    LO_Chla_data_Inter = DF_Date_Range(
        LO_Chla_data_Inter, St_Yr, St_M, St_D, 2010, 10, 19
    )
    ...
    L001_Chla = pd.read_csv(
        f"{input_dir}/water_quality_L001_CHLOROPHYLL-A, CORRECTED.csv"
    )
    L001_Chla.drop(columns=["days"], inplace=True)
    L004_Chla = pd.read_csv(
        f"{input_dir}/water_quality_L004_CHLOROPHYLL-A, CORRECTED.csv"
    )
    L004_Chla.drop(columns=["days"], inplace=True)
    L005_Chla = pd.read_csv(
        f"{input_dir}/water_quality_L005_CHLOROPHYLL-A, CORRECTED.csv"
    )
    L005_Chla.drop(columns=["days"], inplace=True)
    L006_Chla = pd.read_csv(
        f"{input_dir}/water_quality_L006_CHLOROPHYLL-A, CORRECTED.csv"
    )
    L006_Chla.drop(columns=["days"], inplace=True)
    L007_Chla = pd.read_csv(
        f"{input_dir}/water_quality_L007_CHLOROPHYLL-A, CORRECTED.csv"
    )
    L007_Chla.drop(columns=["days"], inplace=True)
    L008_Chla = pd.read_csv(
        f"{input_dir}/water_quality_L008_CHLOROPHYLL-A, CORRECTED.csv"
    )
    L008_Chla.drop(columns=["days"], inplace=True)
    LZ40_Chla = pd.read_csv(
        f"{input_dir}/water_quality_LZ40_CHLOROPHYLL-A, CORRECTED.csv"
    )
    ...
    LO_Chla_N_cols = [
        "L001_CHLOROPHYLL-A, CORRECTED_ug/L",
        "L005_CHLOROPHYLL-A, CORRECTED_ug/L",
        "L008_CHLOROPHYLL-A, CORRECTED_ug/L",
    ]
    LO_Chla["Chla_North"] = LO_Chla[LO_Chla_N_cols].mean(axis=1)
    LO_Chla_S_cols = [
        "L004_CHLOROPHYLL-A, CORRECTED_ug/L",
        "L006_CHLOROPHYLL-A, CORRECTED_ug/L",
        "L007_CHLOROPHYLL-A, CORRECTED_ug/L",
        "L008_CHLOROPHYLL-A, CORRECTED_ug/L",
        "LZ40_CHLOROPHYLL-A, CORRECTED_ug/L",
    ]
    ...
    LO_Chla_LC_N_cols = [
        "L001_CHLOROPHYLL-A(LC)_ug/L",
        "L005_CHLOROPHYLL-A(LC)_ug/L",
        "L008_CHLOROPHYLL-A(LC)_ug/L",
    ]
    LO_Chla_LC["Chla_North"] = LO_Chla_LC[LO_Chla_LC_N_cols].mean(axis=1)
    LO_Chla_LC_S_cols = [
        "L004_CHLOROPHYLL-A(LC)_ug/L",
        "L006_CHLOROPHYLL-A(LC)_ug/L",
        "L007_CHLOROPHYLL-A(LC)_ug/L",
        "L008_CHLOROPHYLL-A(LC)_ug/L",
        "LZ40_CHLOROPHYLL-A(LC)_ug/L",
    ]
    ...
    LO_Chla_Merge[["date", "Chla_N"]].rename(columns={"Chla_N": "Chla"}).to_csv(
        f"{output_dir}/N_Merged_Chla.csv", index=False
    )
    LO_Chla_Merge[["date", "Chla_S"]].rename(columns={"Chla_S": "Chla"}).to_csv(
        f"{output_dir}/S_Merged_Chla.csv", index=False
    )

    # Create Files S65E_Avg_Chla
    S65E_Chla_Inter = pd.read_csv(
        f"{input_dir}/water_quality_S65E_CHLOROPHYLL-A, CORRECTED_Interpolated.csv"
    )
    S65E_Chla_LC_Inter = pd.read_csv(
        f"{input_dir}/water_quality_S65E_CHLOROPHYLL-A(LC)_Interpolated.csv"
    )

    S65E_Chla_Merge = pd.concat([S65E_Chla_Inter, S65E_Chla_LC_Inter]).reset_index(
        drop=True
    )
    S65E_Chla_Merge = S65E_Chla_Merge.drop_duplicates(
        subset="date", keep="first"
    )  # there are duplicates in the data
    # S65E_Chla_Merge.rename(columns={S65E_Chla_Merge.columns[-1]: 'Chla'}, inplace=True)
    S65E_Chla_Merge = S65E_Chla_Merge.loc[
        :, ~S65E_Chla_Merge.columns.str.contains("^Unnamed")
    ]
    ...
    S65_NO = pd.read_csv(
        f"{input_dir}/water_quality_S65E_NITRATE+NITRITE-N_Interpolated.csv"
    )
    S71_NO = pd.read_csv(
        f"{input_dir}/water_quality_S71_NITRATE+NITRITE-N_Interpolated.csv"
    )
    S72_NO = pd.read_csv(
        f"{input_dir}/water_quality_S72_NITRATE+NITRITE-N_Interpolated.csv"
    )
    S84_NO = pd.read_csv(
        f"{input_dir}/water_quality_S84_NITRATE+NITRITE-N_Interpolated.csv"
    )
    S127_NO = pd.read_csv(
        f"{input_dir}/water_quality_S127_NITRATE+NITRITE-N_Interpolated.csv"
    )
    S133_NO = pd.read_csv(
        f"{input_dir}/water_quality_S133_NITRATE+NITRITE-N_Interpolated.csv"
    )
    # S135_NO = pd.read_csv(f'{input_dir}/water_quality_S135_NITRATE+NITRITE-N_Interpolated.csv')
    S154_NO = pd.read_csv(
        f"{input_dir}/water_quality_S154_NITRATE+NITRITE-N_Interpolated.csv"
    )
    S191_NO = pd.read_csv(
        f"{input_dir}/water_quality_S191_NITRATE+NITRITE-N_Interpolated.csv"
    )
    S308_NO = pd.read_csv(
        f"{input_dir}/water_quality_S308C_NITRATE+NITRITE-N_Interpolated.csv"
    )
    FISHP_NO = pd.read_csv(
        f"{input_dir}/water_quality_FECSR78_NITRATE+NITRITE-N_Interpolated.csv"
    )
    L8_NO = pd.read_csv(
        f"{input_dir}/water_quality_CULV10A_NITRATE+NITRITE-N_Interpolated.csv"
    )
    S4_NO = pd.read_csv(
        f"{input_dir}/water_quality_S4_NITRATE+NITRITE-N_Interpolated.csv"
    )
    ...
    NO_Loads_In["S127_NO_Ld"] = (
        Flow_df["S127_In"].values * NO_df["S127_NO"].values * 1000
    )
    NO_Loads_In["S133_NO_Ld"] = (
        Flow_df["S133_P_Q"].values * NO_df["S133_NO"].values * 1000
    )
    # NO_Loads_In['S135_NO_Ld'] = Flow_df['S135_In'].values * NO_df['S135_NO'].values * 1000
    NO_Loads_In["S154_NO_Ld"] = (
        Flow_df["S154_Q"].values * NO_df["S154_NO"].values * 1000
    )
    NO_Loads_In["S191_NO_Ld"] = (
        Flow_df["S191_Q"].values * NO_df["S191_NO"].values * 1000
    )
    NO_Loads_In["S308_NO_Ld"] = (
        Flow_df["S308_In"].values * NO_df["S308_NO"].values * 1000
    )
    NO_Loads_In["FISHP_NO_Ld"] = (
        Flow_df["FISHP_Q"].values * NO_df["FISHP_NO"].values * 1000
    )
    ...
    TP_Loads_In_3MLag_df.to_csv(
        f"{output_dir}/LO_External_Loadings_3MLag.csv", index=False
    )
