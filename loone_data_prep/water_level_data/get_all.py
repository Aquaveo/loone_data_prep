...
D = {
    "LO_Stage": {
        "dbkeys": ["16022", "12509", "12519", "16265", "15611"],
        "datum": "NGVD29",
    },
...
def main(workspace: str, d: dict = D) -> dict:
    ...
    for name, params in d.items():
        ...
        if date_latest is None:
            ...
            if dbhydro_data_is_latest(date_latest):
                ...
                print(
                    f"Downloading of new water level data skipped for {name}. Data is already up to date."
                )
                continue
            ...
            try:
                ...
                print(
                    f"Downloading new water level data for {name} starting from date {date_latest}"
                )
                hydro.get(
                    workspace,
                    name,
                    dbkeys=params["dbkeys"],
    ...
    if os.path.exists(os.path.join(workspace, "LO_Stage.csv")) and os.path.exists(
        os.path.join(workspace, "LO_Stage_2.csv")
    ):
        # Output Progress
        ...
        df_lo_stage_2 = pd.read_csv(
            os.path.join(workspace, "LO_Stage_2.csv"), index_col="date"
        )
        ...
        for i in range(0, len(lo_stage_2_values_navd88)):
            ...
            try:
                lo_stage_2_values_ngvd29.append(
                    _convert_navd88_to_ngvd29(latitude, longitude, value, date.year)
                )
            except Exception as e:
        ...
        if not convert_failure:
            ...
            df_lo_stage = pd.read_csv(
                os.path.join(workspace, "LO_Stage.csv"), index_col="date"
            )
        ...
        convert_failure = True
        print(
            "Error: Missing LO_Stage.csv or LO_Stage_2.csv file, cannot convert and merge."
        )
...
def _convert_navd88_to_ngvd29(
    latitude: float, longitude: float, stage: float, year: int
) -> float:
