...
def main(workspace: str, d: dict = D) -> dict:
    ...
    failed_downloads = (
        []
    )  # List of file names that the script failed to get the latest data for (but the files still exist)
    for name, params in d.items():
        ...
        for station_id in params["station_ids"]:
            station_date_latest[station_id] = find_last_date_in_csv(
                workspace, f"water_quality_{station_id}_{name}.csv"
            )
        ...
        for station_id, date_latest in station_date_latest.items():
            ...
            if date_latest is None:
                ...
                if dbhydro_data_is_latest(date_latest):
                    ...
                    print(
                        f"Downloading of new water quality data for test name: {name} station: {station} skipped. Data is already up to date."
                    )
                    continue

                # Temporarily rename current data file so it isn't over written
                original_file_name = f"water_quality_{station_id}_{name}.csv"
                original_file_name_temp = (
                    f"water_quality_{station_id}_{name}_{uuid.uuid4()}.csv"
                )
                os.rename(
                    os.path.join(workspace, original_file_name),
                    os.path.join(workspace, original_file_name_temp),
                )

                try:
                    # Get only the water quality data that is newer than the latest data in the csv file
                    print(
                        f"Downloading new water quality data for test name: {name} station ID: {station_id} starting from date: {date_latest}."
                    )
                    date_latest = (
                        datetime.strptime(date_latest, "%Y-%m-%d") + timedelta(days=1)
                    ).strftime("%Y-%m-%d")
                    ...
                    if not os.path.exists(os.path.join(workspace, original_file_name)):
                        raise Exception(
                            f"It's possible that the data for test name: {name} station ID: {station_id} has reached its end date."
                        )

                    # Read in the original data
                    df_original = pd.read_csv(
                        os.path.join(workspace, original_file_name_temp), index_col=0
                    )

                    # Calculate the days column for the newly downloaded data
                    df_original_date_min = df_original["date"].min()
                    wq._calculate_days_column(
                        workspace, original_file_name, df_original_date_min
                    )
                    ...
                    # Notify of the error
                    print(
                        f"Error occurred while downloading new water quality data: {e}"
                    )
        ...
        for station in params["station_ids"]:
            if not os.path.exists(
                os.path.join(workspace, f"water_quality_{station}_{name}.csv")
            ):
                missing_files.append(f"water_quality_{station}_{name}.csv")
                print(
                    f"{name} station ID: {station} could not be downloaded after various tries."
                )
    ...
    if missing_files or failed_downloads:
        ...
        if missing_files:
            error_string += (
                f"The following files could not be downloaded: {missing_files}"
            )
