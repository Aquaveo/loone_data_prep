...
def main(workspace: str, d: dict = D, dbkey_stations: dict = DBKEY_STATIONS) -> dict:
    ...
    failed_downloads = (
        []
    )  # List of (data type name, file name) tuples that the script failed to get the latest data for (but the files still exist)
    ...
    for name, params in d.items():
        ...
        for dbkey in params["dbkeys"]:
            ...
            if dbhydro_data_is_latest(date_latest):
                ...
                print(
                    f"Downloading of new {name} data skipped for dbkey {dbkey}. Data is already up to date."
                )
                continue

            # Temporarily rename current data file so it isn't over written
            original_file_name_temp = original_file_name.replace(
                ".csv", f"_{uuid.uuid4()}.csv"
            )
            os.rename(
            ...
            try:
                ...
                print(
                    f"Downloading new {name} data for dbkey {dbkey} starting from date {date_latest}"
                )
                ...
                if not os.path.exists(os.path.join(workspace, original_file_name)):
                    raise Exception(
                        f"It's possible that the data for {name} dbkey {dbkey} has reached its end date."
                    )
